  ...
  function depositMoney() {
    if (!isNaN(deposit) && deposit > 0) {
      ...
      for (const { email, password, balance } of ctx.users) {
        console.log(
          `Checking ${currentemail} ${currentpass} against ${email} ${password}`
        );
      ...
      setStatus(
        "Error: Deposit amount must be greater than $0.00. Please try again."
      );
      setDeposit("");
      setTimeout(() => setStatus(""), 5000);
    } else {
      ...
      body={
        show ? (
          <>
            ...
            to deposit funds and check account balance. <br />
            <br />
            <br />
